...
#include "Foton/Tools/whereami.hxx"
...
bool PythonSystem::start()
{
    ...
    auto loop = [this]() {
        ...
        if (Options::IsIPyKernel())
        {
            ...
            // PyRun_SimpleString(std::string(std::string("if importlib.util.find_spec('Foton') == None:\n\tprint('Sorry, the Foton package isnt on the python path. ") +
            //     std::string("In order to use the python bindings to Foton, please make sure to either run Foton.exe in the same directory as the Foton package, ") +
            //     std::string("or add the directory containing the Foton package to your system path.') ")).c_str());

            PyRun_SimpleString("from ipykernel import kernelapp as app");
            PyRun_SimpleString("app.launch_new_instance()");
#else
            // debug code
            std::cout << "Foton Error: ipykernel mode does not work when Foton is built using debug mode. This is due to a bug with pyzmq. See the following issue for details: https://github.com/zeromq/pyzmq/issues/1245 " << std::endl;
#endif
        }
        else
        {
