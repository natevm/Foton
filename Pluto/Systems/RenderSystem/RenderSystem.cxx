    ...
    auto loop = [](future<void> futureObj) {
        ...
        while (futureObj.wait_for(std::chrono::milliseconds((uint32_t)(100 * (.16 - (currentTime - lastTime))))) == future_status::timeout)
        {
            ...
            if (vulkan->is_initialized()) {
                ...
                if (imageAvailableSemaphore == vk::Semaphore()) imageAvailableSemaphore = device.createSemaphore(semaphoreInfo);
                if (renderCompleteSemaphore == vk::Semaphore()) renderCompleteSemaphore = device.createSemaphore(semaphoreInfo);
                if (presentCompleteSemaphore == vk::Semaphore()) presentCompleteSemaphore = device.createSemaphore(semaphoreInfo);
                    ...
                    if (!Options::IsClient()) {
                        ...
                        for (uint32_t i = 0; i < Entity::GetCount(); ++i) {
                            if (entities[i].is_initialized()) {
                                Material::DrawEntity(maincmd, entities[i]);
                            }
                        }
        ...
        auto device = vulkan->get_device();
        if (device != vk::Device()) {
            if (maincmd != vk::CommandBuffer()) device.freeCommandBuffers(vulkan->get_command_pool(2), maincmd);
            if (imageAvailableSemaphore != vk::Semaphore()) device.destroySemaphore(imageAvailableSemaphore);
            if (renderCompleteSemaphore != vk::Semaphore()) device.destroySemaphore(renderCompleteSemaphore);
            if (presentCompleteSemaphore != vk::Semaphore()) device.destroySemaphore(presentCompleteSemaphore);
        }
    };
